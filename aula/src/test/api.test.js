const request = require("supertest"); // a const chama request pq o supertest permite a requisição; 
//o supertest usa o jest pra fazer os testes
const app = require("../app"); // importamos o app.js porque lá temos a const app = express() 
//que armazena o express, e usamos esse app para fazer a conexão do servidor e aqui ele tem a mesma responsabilidade
...
describe("API test", () => {  //agrupa vários testes
    ...
    test("GET /users/all", (done) => { //descrição do teste unitário como primeiro parâmetro; 
        //função anônima como segundo no done
         request(app) // conexão acontece
            .get("/users/all") //teste fazer um GET, é a requisição
            .expect(200) // Uma função que diz o que esperamos que aconteça, 
            //espero que o status seja 200 e não 500, validação do status code
            .expect((res)=>{
                // console.log("body da resposta", res.body); 
                // mostra no terminal todos os usuarios cadastrados no banco
    ...
    test("POST /users/create", (done) => {
        request(app)
            ...
            .end((err, res) => {
                // console.log("body da resposta", res.body) 
                //os consoles são pra gente ver a res.body mudando
                if(err) return done(err); // se o if tem um único retorno o uso de chaves é facultativo
...
test() ou it() - fazem a mesma coisa, realizam um teste individual
vamos usar o test na aula por ser mais semântico
// primeiro parâmetro é o endpoint: o verbo e a rota, precisa da barra antes, 
precisa da rota raíz e da rota do verbo
